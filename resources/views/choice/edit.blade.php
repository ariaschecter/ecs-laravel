...
<div class="row">
    <div class="col-lg-6">
        <form method="POST" action="{{ url('choice/edit/' . $choice->choice_id) }}">
            @csrf
            <div class="mt-2">
                <label for="question_id" class="form-label">Question</label>
                <select class="form-select" id="question_id" name="question_id">
                        <option selected value="{{ $choice->question_id }}">{{ $choice->question->question }}</option>
            ...
            <div>
                ...
                <input type="text" id="choice_name" name="choice_name" class="form-control @error('choice_name') is-invalid @enderror" value="{{ $choice->choice_name }}" placeholder="List Mapel No">
            </div>
            @error('choice_name') <div class="text-danger">{{ $message }}</div> @enderror

            <div class="mt-2">
                <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="choice_score" name="choice_score" value="1" {{ $choice->choice_score ? 'checked': '' }}>
