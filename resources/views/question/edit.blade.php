...
<div class="row">
    <div class="col-lg-6">
        <form method="POST" action="{{ url('question/edit/' . $question->question_id) }}">
            @csrf
            <div class="mt-2">
                <label for="sub_mapel_id" class="form-label">Select Sub Mapel</label>
                <select class="form-select" id="sub_mapel_id" name="sub_mapel_id">
                        <option value="{{ $question->sub_mapel_id }}">{{ $question->sub_mapel->mapel->mapel_name . ' - ' . $question->sub_mapel->sub_mapel_name }}</option>
            ...
            <div>
                ...
                <input type="text" id="question" name="question" class="form-control @error('question') is-invalid @enderror" value="{{ $question->question }}" placeholder="Sub Mapel No">
            </div>
